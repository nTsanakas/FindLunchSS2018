...
public class FavoritesRestController {
	...
	@ApiResponses(value = { 
			@ApiResponse (code = 200, message = "0 and HTTP OK: success")})
	@RequestMapping(path = "/api/register_favorite/{restaurantId}", method = RequestMethod.PUT)
	public ResponseEntity<Integer> registerFavorite(@ApiParam(name="RestaurantID", value="User has to be logged in", required=true)@PathVariable("restaurantId") Integer restaurantId,
	...
	@PreAuthorize("isAuthenticated()")
	@ApiResponses(value = { 
			@ApiResponse (code = 200, message = "0 and HTTP OK: success | 3 and HTTP CONFLICT: Restaurant for ID not found")})
	@RequestMapping(path = "/api/unregister_favorite/{restaurantId}", method = RequestMethod.DELETE)
	public ResponseEntity<Integer> unregisterFavorite(@ApiParam(name="RestaurantID", value="User has to be logged in", required=true)@PathVariable("restaurantId") Integer restaurantId,
			Principal principal, HttpServletRequest request) {
		LOGGER.info(LogUtils.getInfoStringWithParameterList(request, Thread.currentThread().getStackTrace()[1].getMethodName()));
